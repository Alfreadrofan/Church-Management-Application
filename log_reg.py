...
cur.execute('''CREATE TABLE IF NOT EXISTS record(
            ...
            ''')
cur.execute('''CREATE TABLE IF NOT EXISTS approved(
                    id INTEGER NOT NULL,
                    name text, 
...
def insert_record():
    ...
    if check_counter == 8:        
        try:
            con = sqlite3.connect('userdata.db')
            cur = con.cursor()
            cur.execute("INSERT INTO record (name,email,contact,gender,country,password)VALUES (:name, :email, :contact, :gender, :country, :password)", {
...
def login_response():
    try:
        con = sqlite3.connect('userdata.db')
        c = con.cursor()
        for row in c.execute("Select * from approved"):
            username = row[2]
            pwd = row[6]
